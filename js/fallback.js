...
function displayArticles() {
	...
	var articlesDisplayArea = document.querySelector('#articlesList');
	...
	if (currentTopic === 'popular articles') {
		...
		filteredArticles = rawArticlesList.filter(function (rawArticlesList) {
			return rawArticlesList.topic === currentTopic;
		});
	}

	// Filter if search triggered
	var searchURL = document.location.search;
	var searchString = searchURL.substring(searchURL.indexOf('=') + 1);
	...
	var articlesCount = articlesToDisplay;
...
function changeTopic(button) {
	...
	var resultsHeading = document.querySelector('#articlesHeading');
	...
	switch (currentTopic) {
		case 'account':
			resultsHeading.innerHTML = 'Account';break;
		case 'sports':
			resultsHeading.innerHTML = 'Sports';break;
		case 'casino':
			resultsHeading.innerHTML = 'Casino';break;
		case 'games':
			resultsHeading.innerHTML = 'Games';break;
		case 'exchange':
			resultsHeading.innerHTML = 'Exchange';break;
		case 'poker':
			resultsHeading.innerHTML = 'Poker';break;
		case 'bingo':
			resultsHeading.innerHTML = 'Bingo';break;
		case 'lotto':
			resultsHeading.innerHTML = 'Lotto';break;
		case 'popular articles':
			resultsHeading.innerHTML = 'Popular Articles';break;
		default:
		...
		if (evt.target.matches('button')) {
			switch (evt.target.id) {
				case 'firstPage':
					pageStart = 0;break;
				case 'previousPage':
					pageStart -= articlesToDisplay;break;
				case 'nextPage':
					pageStart += articlesToDisplay;break;
				case 'lastPage':
					pageStart = filteredArticles.length;break;
			}
